...
const ONBOARDING_STEPS = [
  ...
  },
  {
    id: "capture-modes",
    title: "Capture Modes",
    description: "Use one shortcut to start, then choose modes from the toolbar after selecting a region.",
    icon: (
      <svg className="size-8 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path
          strokeLinecap="round"
      ...
      <div className="space-y-6">
          ...
          <div className="p-4 bg-secondary rounded-lg border border-border">
            <div className="flex flex-col items-center gap-2">
              ...
              <div className="text-center">
                <div className="text-sm font-medium text-foreground">Single entry shortcut</div>
                <div className="text-xs text-foreground0 mt-1">Start capture · ⌘⌥⇧R</div>
              </div>
            </div>
          </div>
          <div className="p-4 bg-secondary rounded-lg border border-border">
            <div className="flex flex-col items-center gap-2">
              ...
              <div className="text-center">
                <div className="text-sm font-medium text-foreground">Window / Screen</div>
                <div className="text-xs text-foreground0 mt-1">Choose from toolbar after selection</div>
              </div>
            </div>
          </div>
          <div className="p-4 bg-secondary rounded-lg border border-border">
            <div className="flex flex-col items-center gap-2">
              ...
              <div className="text-center">
                <div className="text-sm font-medium text-foreground">OCR</div>
                <div className="text-xs text-foreground0 mt-1">Extract text from current selection</div>
              </div>
            </div>
          </div>
          <div className="p-4 bg-secondary rounded-lg border border-border">
            <div className="flex flex-col items-center gap-2">
              ...
              <div className="text-center">
                <div className="text-sm font-medium text-foreground">Scroll Capture</div>
                <div className="text-xs text-foreground0 mt-1">Select region, then stitch while scrolling</div>
              </div>
            </div>
          </div>
        </div>
        <div className="space-y-3">
          <div className="flex items-center justify-between p-3 bg-secondary rounded-lg border border-border">
            <span className="text-sm text-foreground">Start capture session</span>
          ...
          <p className="text-xs text-foreground0 text-center pt-2">
            Window / Screen / OCR / Scroll are now selected from the toolbar after region selection
          </p>
        </div>
      </div>
    ),
  ...
  },
  {
    ...
    content: (
      <div className="space-y-4">
        ...
        <div className="p-4 bg-secondary rounded-lg border border-border">
          <div className="text-sm font-medium text-foreground mb-1">Clipboard copy</div>
          <p className="text-xs text-muted-foreground text-pretty">
            Saved screenshots are copied to your clipboard so they are ready to paste.
          </p>
        </div>
      </div>
    ),
    ...
    content: (
      <div className="space-y-4 text-center">
        <p className="text-sm text-muted-foreground text-pretty">
          Press <kbd className="px-2 py-1 bg-secondary border border-border rounded text-foreground font-mono text-xs tabular-nums">⌘⌥⇧R</kbd> to open capture overlay, then select an area/window/screen.
        </p>
        <div className="p-4 bg-secondary/50 rounded-lg border border-border/50">
          <p className="text-xs text-foreground0 text-pretty">
            <span className="text-muted-foreground font-medium">Pro tip:</span> Use the bottom toolbar after selection to jump into scroll capture or finish directly into the editor.
          </p>
        </div>
      </div>
    ),
...
export function OnboardingFlow({ onComplete }: OnboardingFlowProps) {
  ...
  const localizedTitle =
    step.id === "welcome" ? t("onboarding.welcome.title")
    : step.id === "capture-modes" ? t("onboarding.capture.title")
    : step.id === "settings" ? t("onboarding.settings.title")
    : step.id === "permissions" ? t("onboarding.permissions.title")
    : step.id === "ready" ? t("onboarding.ready.title")
    : step.title;
  const localizedDescription =
    step.id === "welcome" ? t("onboarding.welcome.desc")
    : step.id === "capture-modes" ? t("onboarding.capture.desc")
    : step.id === "settings" ? t("onboarding.settings.desc")
    : step.id === "permissions" ? t("onboarding.permissions.desc")
    : step.id === "ready" ? t("onboarding.ready.desc")
    : step.description;
  const isFirstStep = currentStep === 0;
  const isLastStep = currentStep === ONBOARDING_STEPS.length - 1;

  return (
    <div className="fixed inset-0 z-50 bg-background/95 backdrop-blur-sm overflow-y-auto p-4 sm:p-8">
      <div className="w-full max-w-2xl mx-auto">
        <OnboardingProgress currentStep={currentStep + 1} totalSteps={ONBOARDING_STEPS.length} />
        <Card className="bg-card border-border">
          <CardContent className="p-4 sm:p-8 flex flex-col max-h-[calc(100dvh-7rem)]">
            ...
            <div className="flex items-center justify-between mt-6 pt-6 border-t border-border bg-card">
              <div>
                {!isFirstStep && (
                  <Button
                    variant="outline"
