...
export function RegionSelector({ onSelect, onCancel, instructionText = "Drag to select · ESC to cancel", monitorShots }: RegionSelectorProps) {
  ...
  useEffect(() => {
    ...
    const toRelativePoint = (e: MouseEvent) => {
      const rect = container.getBoundingClientRect();
      const x = Math.min(Math.max(0, e.clientX - rect.left), bounds.width);
      const y = Math.min(Math.max(0, e.clientY - rect.top), bounds.height);
      return { x, y };
    };
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      currentRef.current = toRelativePoint(e);
      needsUpdateRef.current = true;
    };
  ...
  }, [bounds.height, bounds.minX, bounds.minY, bounds.width, onSelect, onCancel]);
