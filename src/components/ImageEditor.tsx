...
export function ImageEditor({ imagePath, onSave, onCancel }: ImageEditorProps) {
  ...
  const handleCopy = useCallback(async () => {
    ...
    try {
      ...
      if (saved.copy_warning) {
        toast.error(t("app.toast.copyWarn"), {
          description: saved.copy_warning,
          duration: 3000,
        });
      }
  ...
  }, [screenshotImage, annotations, renderHighQualityCanvas, isSaving, isCopying, tempDir, imagePath, t]);
    ...
    <div className="flex flex-col h-dvh bg-background text-foreground">
      <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-card">
        <div className="flex items-center gap-4">
          <h2 className="text-xl font-semibold text-foreground text-balance">{t("editor.title")}</h2>
          <TooltipProvider>
        ...
        <div className="flex-1 flex items-center justify-center p-6 bg-background overflow-hidden min-w-0 min-h-0">
          <div className="w-full h-full flex items-center justify-center min-w-0 min-h-0">
            {previewUrl ? (
              ...
              <div className="text-muted-foreground text-base text-pretty">{t("editor.generating")}</div>
            ) : error ? (
              <div className="text-center text-red-400 p-5">
                <p className="mb-2 text-base font-medium text-balance">{t("editor.loadFailed")}</p>
                <small className="text-foreground0 text-xs text-pretty">{error}</small>
              </div>
            ) : (
              <div className="text-muted-foreground text-base text-pretty">{t("editor.loading")}</div>
            )}
